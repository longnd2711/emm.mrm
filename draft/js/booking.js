...
function handleMeetingClick(rowIndex) {
    ...
    html += `<button onclick="closeModals()" class="flex-1 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold transition-colors">Đóng</button>
        </div>`;
        ...
        <div class="mb-5">
            ...
            <div class="bg-slate-50 p-4 rounded-xl border border-slate-100 space-y-3 text-sm">
                <div class="flex flex-col">
                    <span class="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1">
                        ${prefillTimeStr ? `Thời gian: ${prefillTimeStr}` : 'Hiện tại'}
                    </span>
...
// Hàm kiểm tra để hiển thị nút Lặp lại nhiều ngày
